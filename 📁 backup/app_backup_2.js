...
function getSavedCardImage(c) {
  const record = state.records[c.no];
  return record && record.image ? record.image : "";
}

function getCardImageSource(c) {
  // Future-proof: replace this with AI-generated card artwork source.
  return getSavedCardImage(c);
}

function getCurrentCardImageForSave() {
  return selectedPhoto || "";
}

function saveCardImage(no, imageSrc, mimeType = "image/jpeg") {
  if (!imageSrc) return;
  const record = state.records[no] || { count: 0, first: new Date().toLocaleDateString('ja-JP'), memo: '' };
  record.image = imageSrc;
  record.imageMime = mimeType;
  record.imageSavedAt = new Date().toISOString();
  state.records[no] = record;
  saveState();
}
...
function getCategoryPlaceholder(c) {
  // simple category-based placeholder (emoji + text hidden for accessibility)
  const emoji = getCreatureEmoji(c);
  const cat = (c.category || 'other').replace(/"|'/g, '');
  return `<div class="card-placeholder" data-category="${cat}"><div class="card-placeholder-emoji">${emoji}</div><div class="card-placeholder-text">${cat}</div></div>`;
}

function getCardImage(c, owned = false) {
  const src = getCardImageSource(c);
  const placeholder = getCategoryPlaceholder(c);
  if (!src) return placeholder;
  const cls = `card-image`;
  const img = `<img class="${cls}" src="${src}" alt="${c.name}" loading="lazy" style="display:none;" onerror="this.onerror=null;this.style.display='none';this.previousElementSibling.style.display='grid';" onload="this.previousElementSibling.style.display='none';this.style.display='block';">`;
  return placeholder + img;
}
function getCardTheme(c) {
  const category = String(c.category || '');
  if (category.includes('昆虫')) return { accent: '#f4c75a', bg1: '#23412d', bg2: '#0f1912', line: '#d8a93d', text: '#fff6d8' };
  if (category.includes('魚')) return { accent: '#d4eaf7', bg1: '#21506f', bg2: '#071624', line: '#89b7d2', text: '#f4fbff' };
  if (category.includes('鳥')) return { accent: '#e7f7ff', bg1: '#2f5d87', bg2: '#0d1c2b', line: '#8fcfff', text: '#f5fcff' };
  if (category.includes('両生') || category.includes('爬虫')) return { accent: '#f2d77c', bg1: '#4b2c69', bg2: '#170d1d', line: '#b18ed8', text: '#fff7dc' };
  if (category.includes('哺乳')) return { accent: '#d8aa60', bg1: '#6b3f1f', bg2: '#231009', line: '#e9bf74', text: '#fff7e8' };
  if (category.includes('植物')) return { accent: '#dceec4', bg1: '#2d6238', bg2: '#0a1d14', line: '#88bf6b', text: '#f7fce8' };
  if (category.includes('きのこ') || category.includes('木の実')) return { accent: '#ffb05a', bg1: '#7a4018', bg2: '#2a1408', line: '#ffcf86', text: '#fff6e8' };
  return { accent: '#9acb92', bg1: '#23462f', bg2: '#0a1713', line: '#8ec08c', text: '#f8fff4' };
}
function cardHtml(c, force = false) {
  const owned = Boolean(force || state.owned[c.no]);
  const rarity = String(c.rarity || "C").toUpperCase();
  const rarityDisplay = owned ? rarity : "?";
  const no = `No.${String(c.no).padStart(3, "0")}`;
  const imgHtml = getCardImage(c, owned);
  const theme = getCardTheme(c);

  return `
  <div class="card ${owned ? 'card--owned' : 'card--locked'} ${rarity}"
       data-no="${c.no}"
       style="
    ...
    <div class="card-frame">
      ...
      <div class="card-info">
        ...
        <div class="card-discoverer">
          👤 ${owned ? state.spiritName : "-----"}
        </div>
...
function register(no, memo = '', imageSrc = null, imageMime = 'image/jpeg') {
  const before = getStage();
  const c = creatures.find(x => x.no === Number(no));
  if (!c) return;
  const isNew = !state.owned[c.no];
  const r = state.records[c.no] || { count: 0, first: new Date().toLocaleDateString('ja-JP'), memo: '' };
  state.owned[c.no] = true;
  r.count++;
  if (memo && !r.memo) r.memo = memo;
  state.records[c.no] = r;
  if (imageSrc) {
    saveCardImage(no, imageSrc, imageMime);
  } else {
    saveState();
  }

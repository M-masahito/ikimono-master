...
function openDeveloperTestPanel({
    ...
    judgeResult.innerHTML = `
        ...
        <section class="card-get-result">
            ...
            <button
    id="developerResetTestSaveButton"
    class="subButton"
    type="button"
>
    🔄 テストデータをリセット
...
const resetTestSaveButton =
    judgeResult.querySelector(
        "#developerResetTestSaveButton"
    );
if (emblemTestButton) {
...
if (resetTestSaveButton) {
    resetTestSaveButton.addEventListener("click", () => {

        const confirmed =
            window.confirm(
                "テスト中の発見・エンブレム・スピリアをリセットします。\n本番セーブのバックアップは残ります。"
            );
...
const emblemTypeId = discoveredItem?.typeId;
...
const emblemStages =
    Array.isArray(emblemMaster.stages)
        ? emblemMaster.stages
        : [];
...
id: emblemTypeId,
typeId: emblemTypeId,
name: `${emblemName}のエンブレム`,            rank: rankId,
            stage: targetStage,
            discoveredCount: emblemCount,
            unlockedAt:
                new Date().toISOString()
        });
        ...
        Array.isArray(save.spiria)
            ? save.spiria.filter(
                item =>
                    !(
                        item === emblemTypeId ||
                        item?.id === emblemTypeId ||
                        item?.spiriaId === emblemTypeId
                    )
            )
            : [];
    ...
    return new Promise(resolve => {
        ...
        judgeResult.innerHTML = `
            <section style="min-height:72vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:28px 18px;background:radial-gradient(circle at center,#54231f 0%,#240d14 58%,#10070b 100%);color:#fff;border-radius:24px;overflow:hidden;">
                ...
                <h2 style="margin:10px 0 4px;font-size:28px;">${escapeHtml(reward.emblemName)} GET！</h2>
                <p style="margin:0;color:#eadccf;">${escapeHtml(reward.rankName)}ランク</p>
                <img src="${escapeHtml(reward.emblemImage)}" alt="${escapeHtml(reward.emblemName)}" style="display:block;width:min(82vw,360px);height:min(82vw,360px);object-fit:contain;margin:10px auto;filter:drop-shadow(0 14px 22px rgba(0,0,0,.55));animation:emblemGetPop .65s ease-out both;">
               <strong style="font-size:21px;color:#9fe8ff;">${escapeHtml(reward.emblemName)}が輝きだした！</strong>
...
const emblemScreen =
    judgeResult.querySelector("section");
...
emblemScreen.style.cursor = "pointer";

const tapGuide =
    document.createElement("div");

tapGuide.textContent =
    "タップしてスピリア解放へ";

tapGuide.style.cssText = `
    margin-top:18px;
    font-size:14px;
    font-weight:bold;
    color:#fff;
    animation:emblemTapBlink 1.2s ease-in-out infinite;
`;

emblemScreen.appendChild(tapGuide);

const style =
    document.createElement("style");

style.textContent = `
    @keyframes emblemTapBlink {
        0%, 100% { opacity:.45; }
        50% { opacity:1; }
    }
`;

emblemScreen.appendChild(style);

emblemScreen.addEventListener(
    "click",
    () => {
        resolve();
    },
